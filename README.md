<div align="center">
  <img width="180" height="180" src="public/logo512.png" alt="logo">
  <h1><a href="https://duodmv.851.workers.dev/" target="_blank"><b>DuoDMV</b></a></h1>
  <p>Duolingo but for your driver's permit.<br>Study California's driving rules and pass your knowledge test.</p>
</div>

## 🖼️ Showcase

|                        Home                         |                            Profile                            |
| :-------------------------------------------------: | :-----------------------------------------------------------: |
|         ![Home](docs/screenshots/home.png)          |           ![Profile](docs/screenshots/profile.png)            |
|                     **Lessons**                     |                        **Learn Mode**                         |
|    ![Lessons](docs/screenshots/lesson-list.png)     |        ![Learn Mode](docs/screenshots/learn-mode.png)         |
|                 **True/False Quiz**                 |                   **Multiple Choice Quiz**                    |
| ![True/False](docs/screenshots/quiz-true-false.png) | ![Multiple Choice](docs/screenshots/quiz-multiple-choice.png) |

## 🛠️ Tech Stack

- [Bun](https://bun.com) - Package Manager
- [Claude Code](https://www.claude.com/product/claude-code) - AI Coding Agent
- [Tanstack Start](https://tanstack.com/start/latest) - React Framework
- [Cloudflare Workers](https://workers.cloudflare.com/) - Hosting

## 📜 License

This project is released under the MIT License. See [LICENSE](LICENSE.md) for details.

## 🌟 Support

If you like this project, please consider giving it a star. ️
