...
export function TrueFalse({ question, onAnswer, disabled = false }: TrueFalseProps) {
  ...
  const getButtonClass = (buttonValue: boolean) => {
    const baseClass =
      "flex flex-col items-center justify-center gap-3 p-6 rounded-xl transition-all duration-100 font-bold text-lg";
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold text-zinc-900 leading-relaxed">{question.statement}</h2>
        ...
        >
          <kbd
            className={clsx(
              ...
              hasAnswered ? "bg-surface-200 text-surface-400" : "bg-surface-200 text-surface-500",
            )}
          >
            T
          </kbd>
